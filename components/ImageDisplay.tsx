...
export const ImageDisplay: React.FC<ImageDisplayProps> = ({ imageSrc, onSelect, isEnhancing, historicalSelection, useFixedSelectionBox, fixedSelectionSizePercentage, contrast }) => {
  ...
  const draw = useCallback(() => {
    ...
    ctx.save(); // Save the current state (without any filters)
    ...
    ctx.restore(); // Restore to the state before applying the filter
  ...
  }, [image, selection, getCanvasScale, historicalSelection, contrast]);

  // Redraw whenever contrast changes
  useEffect(() => {
    draw();
  }, [contrast, draw]);
